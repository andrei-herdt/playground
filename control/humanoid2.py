...
def compute_des_acc(t, ref, gains, state, data, nu, nv1, vmapu):
    ...
    des_acc['R_root'] = ddotR_d(data.qpos[3:7], data.qvel[3:6], ref['R_d_root'], np.zeros(3), gains['Kp_q'], gains['Kd_q'])
    return des_acc
...
def setupQPSparseFullFullJacTwoArms(M1, M2, h1, h2, Jc, jacs, ee_ids, vmapu, weights, des_acc, nv1, nu, nforce, qp, qpproblem):
    ...
    qpproblem.H[ntau+nv1:ntau+nv1+nu, ntau+nv1:ntau+nv1+nu] += J5.T@W5@J5 # ddq_2
    # [forces]

...
def ddotq_d_full(
    p, v, p_delta, v_delta, p_d_root, R_d_root, q2_d, v_d, Kp_q, Kd_q, vmapu
):
    angerr = np.zeros(3)
    ang = p[3:7]
...
def setupQPDense(
    M2, J1, J2, J4, W1, W2, W3, W4, h2, ref1, ref2, ref4, nu, nforce, qp, qpproblem
):
...
def setupQPSparseFull(
    ...
    H[:nu, :nu] += W3  # tau
    H[ntau + nv0 : ntau + nv0 + nu, ntau + nv0 : ntau + nv0 + nu] += (
        J1.T @ W1 @ J1
    )  # ddq_2
    H[ntau + nv0 : ntau + nv0 + nu, ntau + nv0 : ntau + nv0 + nu] += (
        J4.T @ W4 @ J4
...
def get_dynamics(
    model: Any,  # type of model should be specified if known
